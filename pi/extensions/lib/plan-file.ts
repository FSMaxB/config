...
// overwriting it; the path-permission engine treats it as the one writable plan path.
// Parked on globalThis rather than in a module binding because pi loads every extension with
// its own jiti instance and no module cache, so plan mode and the file tools would each get a
// private copy of this module and never see one another's writes.
