...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("thinking", {
    ...
    getArgumentCompletions: (prefix): AutocompleteItem[] | null => {
      const matches = supportedLevels(model).filter((level) =>
        level.startsWith(prefix),
      );
    ...
    handler: async (args, ctx) => {
      ...
      if (levels.length < 2) {
        ctx.ui.notify(
          `${model?.id ?? "This model"} has no selectable thinking levels`,
          "warning",
        );
        return;
      }
      ...
      if (requested) {
        ...
        if (!level) {
          ctx.ui.notify(
            `Unknown level "${requested}". Available: ${levels.join(", ")}`,
            "error",
          );
          return;
        }
