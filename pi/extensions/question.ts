...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const summary =
        outcome.kind === "freeform"
          ? outcome.text
          : outcome.selections.join(", ");
      const unresolved =
        outcome.kind === "selection" ? (outcome.unresolved ?? []) : [];
      const warning =
        unresolved.length > 0
    ...
    renderCall(args, theme) {
      ...
      if (rawOptions.length > 0) {
        const titles = rawOptions.map(
          (option: unknown) => coerceOption(option)?.title ?? "<invalid>",
        );
        text +=
          "\n" +
          theme.fg(
            "dim",
            `  ${rawOptions.length} option(s): ${titles.join(", ")}`,
          );
      }
...
async function askSingle(
  ...
  const displays = options.map(({ title, description }) =>
...
async function askMultiple(
  ...
  if (selections.length > 0)
    return { kind: "selection", selections, unresolved };
...
function resolveToken(token: string, options: QuestionOption[]): string[] {
  ...
  if (range) {
    ...
    if (first >= 1 && last <= titles.length)
      return titles.slice(first - 1, last);
  }

  const position = Number(token);
  if (Number.isInteger(position) && position >= 1 && position <= titles.length)
    return [titles[position - 1]];

  const match = titles.find(
    (title) => title.toLowerCase() === token.toLowerCase(),
  );
  return match ? [match] : [];
}
...
function coerceOption(option: unknown): QuestionOption | null {
  ...
  if (option && typeof option === "object") {
    const { title, description } = option as {
      title?: unknown;
      description?: unknown;
    };
    if (typeof title === "string" && title.trim()) {
      ...
      return trimmedDescription
        ? { title: title.trim(), description: trimmedDescription }
        : { title: title.trim() };
    }
  }
  return null;
