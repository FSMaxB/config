...
export function loadPolicyConfig(directory: string): SubagentModelConfig {
	try {
		const parsed = JSON.parse(fs.readFileSync(path.join(directory, "config.json"), "utf-8"));
...
function isLocalModel(model: Model<Api>, config: SubagentModelConfig): boolean {
	...
	try {
		...
		return (
			host === "localhost" || host === "::1" || host === "[::1]" || host === "0.0.0.0" || host.startsWith("127.")
		);
	} catch {
		return false;
	}
...
function sameProviderFamily(a: string, b: string, config: SubagentModelConfig): boolean {
	if (a === b) return true;
	return (config.providerGroups ?? []).some((group) => group.includes(a) && group.includes(b));
}
