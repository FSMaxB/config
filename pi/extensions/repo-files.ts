...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool(
		scoped(createLsToolDefinition(cwd), {
			...
			label: "Repo ls",
			mode: "read",
			guideline: "Use repo_ls to list a directory instead of shelling out to ls.",
		}),
	);
	...
	pi.registerTool(
		scoped(createFindToolDefinition(cwd), {
			...
			label: "Repo find",
			mode: "read",
			guideline: "Use repo_find to locate files by glob instead of shelling out to find or fd.",
		}),
	);
	...
	pi.registerTool(
		scoped(createGrepToolDefinition(cwd), {
			...
			label: "Repo grep",
			mode: "read",
			guideline: "Use repo_grep to search file contents instead of shelling out to grep or rg.",
		}),
	);
	...
	pi.registerTool(
		scoped(createWriteToolDefinition(cwd), {
			...
			label: "Repo write",
			mode: "write",
			guideline: "Use repo_write to create a file instead of shelling out to a heredoc.",
		}),
	);
	...
	pi.registerTool(
		scoped(createEditToolDefinition(cwd), {
			...
			label: "Repo edit",
			mode: "write",
			guideline: "Use repo_edit to change an existing file instead of rewriting it wholesale.",
		}),
	);
}
...
function scoped(definition: ToolDefinition<any, any, any>, options: ScopeOptions): ToolDefinition<any, any, any> {
